...
// RespondTemplate ends template to the client.
func RespondTemplate(ctx context.Context, w http.ResponseWriter, name string, data *template.Template, statusCode int) error {
	ctx, span := trace.SpanFromContext(ctx).Tracer().Start(ctx, "foundation.web.RespondTemplate")
	defer span.End()
	...
	// Send the result back to the client.
	if err := data.ExecuteTemplate(w, name, nil); err != nil {
		return err
	}
...
func RespondError(ctx context.Context, w http.ResponseWriter, err error) error {
	ctx, span := trace.SpanFromContext(ctx).Tracer().Start(ctx, "foundation.web.RespondError")
	defer span.End()
